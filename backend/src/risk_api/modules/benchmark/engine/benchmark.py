...
def run(data, output, epochs=80, seeds=(42,)):
    ...
    variants: dict[str, dict[str, Any]] = {
        "full": dict(),
        "self_only": dict(mode="self_only", use_prior=False),
        "no_hyper": dict(mode="no_hyper"),
        "no_graph": dict(mode="no_graph"),
        "no_prior": dict(use_prior=False),
        "pretrained": dict(pretrain_epochs=20),
    }
    rows = []
    for seed in seeds:
            ...
            rows.append(
                {
                    ...
                    "test": report["model"]["test"],
                    "valid": report["model"]["valid"],
    ...
    selected = min(
        variants,
        key=lambda name: (
            sum(r["validation_loss"] for r in rows if r["variant"] == name) / len(seeds)
        ),
    )
    result = {
        "rows": rows,
        "selected_variant": selected,
        "selection_rule": "lowest mean validation BCE over seeds",
    ...
    for r in rows:
        m = r["test"]
        lines.append(
            f"| {r['variant']} | {r['seed']} | {m['roc_auc']:.4f} | {m['pr_auc']:.4f} | "
            f"{m['ks']:.4f} | {m['brier']:.4f} | {m['lift_at_10pct']:.4f} | "
            f"{m['capture_at_5pct']:.4f} |"
        )
    lines += [
        "",
        f"按验证集损失选择：{selected}。未按测试成绩选择模型。",
        "",
