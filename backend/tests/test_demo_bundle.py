...
def test_shared_bundle_checksums_and_portable_api(tmp_path):
    ...
    shutil.copytree(ROOT / 'backend/comrisk', tmp_path / 'backend/comrisk',
...
with TestClient(app) as client:
    ...
    prediction = client.post('/v1/predict', json={'company_ids':['C00010']}).json()['predictions'][0]
    explanation = client.get('/v1/explain/C00010').json()
    assert detail['risk_probability'] == prediction['risk_probability'] == explanation['risk_probability']
