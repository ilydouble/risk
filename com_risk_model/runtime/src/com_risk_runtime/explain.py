...
def explain(predictor, data, company_id):
    index = next(
        (i for i, n in enumerate(data.nodes) if n.id == company_id and n.kind == "company"), None
    )
    if index is None:
    ...
    if predictor.meta["prior"]:
        add_prior(graph, data, predictor.meta["prior"])
    contributions = []
    with torch.no_grad():
        for j, name in enumerate(data.feature_names):
            ...
            p = (
                (
                    predictor.model(masked, predictor.meta["mode"])[index]
                    + predictor.meta["calibration"]["intercept"]
                )
                .sigmoid()
                .item()
            )
            contributions.append(
                {
                    "feature": name,
                    "value": data.nodes[index].features[j],
                    "probability_without_feature": p,
                    "probability_delta": base - p,
                }
            )
